...
//   if(single.sound_flag == TRUE)
//   {
//       [touVoice setImage:[UIImage imageNamed:@"kaiguan1.png"]];
//   }
//    if(single.vibrate_flag == TRUE)
//    {
//        [touShake setImage:[UIImage imageNamed:@"kaiguan1.png"]];
//    }
    
    settings = [[Settings alloc] init];
    NSArray *array = [settings readFromLocalFile];
    if(array != nil)
    {
        BOOL f_sound = [[array objectAtIndex:0] boolValue];
        BOOL f_vibrate = [[array objectAtIndex:1] boolValue];
        BOOL f_frame = [[array objectAtIndex:2] boolValue];
//        BOOL f_frame = YES;
        if(f_sound == TRUE)
        {
            [touVoice setImage:[UIImage imageNamed:@"kaiguan1.png"]];
        ...
        if(f_frame == TRUE)
        {
            [touFrame setImage:[UIImage imageNamed:@"kaiguan1.png"]];
        }
        else
        {
...
//        float x = gyroData.rotationRate.x*9.8*5;
//        float y = gyroData.rotationRate.y*9.8*5;
//        float z = gyroData.rotationRate.z*9.8*5;
//
//        sendMouseEvent(2, -z, -x, single.cur_tvInfo.tv_ip, [single.cur_tvInfo.udpPort intValue]);
//    }
//}
...
//    UIImageView *vacancy2 =(UIImageView*)[self.view viewWithTag:14];
    ...
    if(CGRectContainsPoint([vacancy1 frame], pt))
    {
        ...
        sendPSPEvent(0, "1006", 0, single.cur_tvInfo.tv_ip);
        sendPSPEvent(1, "1006", 0, single.cur_tvInfo.tv_ip);
    }
...
-(void)lockerViewMonitor:(UITouch *)touch           //对抽屉进行监听
{
    ...
    if(CGRectContainsPoint(frame, pt))
    {
        single.frame_flag = FALSE;
        [touFrame setImage:[UIImage imageNamed:@"kaiguan2.png"]];
    }
    [settings writeToLocalFile];
    ...
    if(motionManager.gyroAvailable)
    {
         ...
         ^(CMGyroData *gyroData, NSError *error)
        {
            ...
            else
            {
                ...
                x = gyroData.rotationRate.x*9.8*5.8;
                y = gyroData.rotationRate.y*9.8*5.8;
                z = gyroData.rotationRate.z*9.8*5.8;
            }
//            pthread_t pid = pthread_self();
//            printf("pid== %d", pid);
            if(flag)
            {
                sendMouseEvent(2, -z*2.5, -x*1.5, single.cur_tvInfo.tv_ip, [single.cur_tvInfo.udpPort intValue]);
            }
        }];
    }
    else
...
{
    sendPSPEvent(0, "2009", 0, single.cur_tvInfo.tv_ip);
